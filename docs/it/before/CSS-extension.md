...
CSS 预处理器定义了一种新的语言，其基本思想是，用一种专门的编程语言，为 CSS 增加一些编程特性，将 CSS 作为目标生成文件，然后开发者只要用这种语言进行编码工作。
...
调用混入时，如果传入了参数则使用传入的参数，否则使用默认值。当混入没有定义默认值时，调用时必须传入参数，否则编译会报错。
...
CSS 预处理器是对 CSS 语言能力的扩展与增强，它能让代码更加简洁、可读性更强、层级关系更加清晰，同时易于维护，大幅减少重复、枯燥的 CSS 编写工作。
