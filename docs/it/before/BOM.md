...
BOM（Browser Object Model 浏览器对象模型），BOM 提供了很多对象，用于访问浏览器的功能。这些功能都是由浏览器厂商所提供的，没有一个统一的标准，每个浏览器的 BOM 操作可能有一定的差异。BOM 的很多操作权限都很高（例如关闭浏览器窗口），可能某些浏览器提供商会禁止使用这些功能，因此在实际开发中应该尽量减少 BOM 操作。
...
所有的浏览器都支持 window 对象。在浏览器中，window 对象有着双重角色：它既是 JavaScript 访问浏览器窗口的接口，也是 ECMAScript 规定的全局（Global）对象。
...
7. `protocol "http:"` 返回页面使用的协议，通常是 `http:` 或 `https:`
8. `search "?q=javascript"` 返回 URL 的查询字符串，该字符串以问号开头。可通过 `console.log(location.search)` 获取
